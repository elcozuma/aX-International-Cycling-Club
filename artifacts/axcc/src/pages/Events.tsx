...
const expeditions = [
  {
    ...
    costNote3: "Group discounts available on the Event Fee for 5 or more riders booking together",
                ...
                >
                  ...
                  <div className="relative z-10 flex flex-col h-full p-5" style={{ minHeight: "280px" }}>
                    <h2
                      ...
                      style={{ ...rubikOne, textShadow: "0 1px 8px rgba(0,0,0,0.85), 0 0px 2px rgba(0,0,0,1)" }}
                    >
                      {exp.title}
                    </h2>
                    ...
                    <div className="mt-4 space-y-1.5 bg-black/30 rounded-md px-3 py-2.5" style={nunito}>
                      ...
                      <div className="pt-1.5 mt-0.5 border-t border-white/15">
                        ...
                        <p className="text-white/40 text-xs uppercase tracking-widest my-0.5">or</p>
                        <p className="text-white text-xs font-semibold">{exp.cost2}</p>
                        <p className="text-white/50 text-xs italic mt-1">{exp.costNote}</p>
                        <p className="text-white/50 text-xs italic mt-0.5">{exp.costNote2}</p>
                        <p className="text-accent/70 text-xs italic mt-1.5">{exp.costNote3}</p>
                      </div>
                    </div>

                    {/* Buttons — equal width via grid */}
                    <div className="mt-4 grid grid-cols-2 gap-2">
                      <Link href={exp.detailPath} className="block">
                        <span
                          className="block w-full py-2 text-center bg-accent/15 border border-accent/50 text-accent hover:bg-accent hover:text-background transition-colors uppercase text-xs rounded tracking-widest cursor-pointer"
                          style={rubikOne}
                        >
                          VIEW EXPEDITION
                        </span>
                      ...
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="block w-full py-2 text-center border border-white/25 text-white/60 hover:border-accent/50 hover:text-accent transition-colors uppercase text-xs rounded tracking-widest"
                        style={rubikOne}
